...
The `home-routes` handler in the `luminus-chunked-response.routes.home` namespace
...
The `home-page` function will in turn call the `luminus-chunked-response.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `luminus-chunked-response.handler` namespace:
...
Request middleware functions are located under the `luminus-chunked-response.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `luminus-chunked-response.dev-middleware` namespace found in
